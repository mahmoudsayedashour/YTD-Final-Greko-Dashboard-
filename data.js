...
async function buildParsed() {
  console.log('[api/data] Fetching workbook …');
  let res;
  try {
  ...
  try {
    ...
    throw new Error(`Failed to read arrayBuffer from response: ${e.message}`);
  }
  
  console.log('[api/data] Parsing workbook …');
  let wb;
  try {
    wb = XLSX.read(buf, { type: 'buffer', cellDates: true });
  } catch(e) {
    throw new Error(`XLSX.read failed on buffer (Size: ${buf.length} bytes): ${e.message}`);
  }

  // ── 1. Lookup maps (built once) ────────────────────────────────
  const productMap  = {}; // code → product name
  const categoryMap = {}; // code → category
  const channelMap  = {}; // customer name → channel
  ...
  // ── 3. Parse Actual sheets (identical logic for both years) ────
  function parseActual(sheetName) {
    const sheet = wb.Sheets[sheetName];
    if (!sheet) { console.warn(`[api/data] Sheet "${sheetName}" not found`); return []; }
    try {
      ...
      for (let i = 0; i < rows.length; i++) {
        ...
        try {
          ...
          throw new Error(`Row ${i+1}: ${err.message} (Data: ${JSON.stringify(r).substring(0,100)}...)`);
        }
      }
      return out;
    } catch(e) {
  ...
  console.log('[api/data] Processing Actual 25 …');
  const rows25 = parseActual('Actual 25');
  console.log('[api/data] Processing Actual 2026 …');
...
async function getParsed() {
  if (!_parsed || (Date.now() - _cacheTime > CACHE_TTL)) {
    try {
      ...
      throw new Error(`buildParsed failed: ${e.message}`);
    }
  }
  return _parsed;
}
